...
if (source !== "floraison depot" && destination !== "floraison depot") {
  return res.status(400).json({ error: "Floraison Depot Obligatoire Source or Destination" });
}
if (source === destination) {
...
if (entreDechet <= 0) {
  return res.status(400).json({ error: "Invalid Quantity" });
    ...
    const nlotDocument = await NlotdModel.findOne({ Nlot: nlot });
    ...
    if (existingEntry) {
      ...
      if (source === "floraison depot") {
        // If source is "floraison depot," update as sortie
      ...
      const newEntry = {
        ...
        entre: source !== "floraison depot" ? Math.max(parseInt(entreDechet), 0) : 0,
        sortie: source === "floraison depot" ? Math.max(parseInt(entreDechet), 0) : 0,
        source: source,
    ...
    const pfiniDocument = await PfinidModel.findOne({ CodeArticle: nlotDocument.CodeArticle });
...
router.get("/allnlots", async (req, res) => {
  try {
    ...
    const nlots = await NlotdModel.find({}); // Query the Nlot collection to get all Nlots
...
router.get('/bon', async (req, res, next) => {
  try {
    ...
    // Query the NlotdModel to find dailyEntries matching the date, source, and destination
    const dailyEntries = await NlotdModel.find({
...
router.get("/all", async (req, res) => {
  try {
    // Fetch all records from the NlotdModel
    const result = await NlotdModel.find({});
console.log(result)
...
console.log("Min = ", Min)
    ...
    const Pfinid = await PfinidModel.findOne({ CodeArticle });

    if (Pfinid) {
      const NlotFind = await NlotdModel.findOne({ Nlot });

      if (!NlotFind) {
        const newNlot = new NlotdModel({
          Nlot,
        ...
        // Add the newNlot to the PfinidModel's nlots array
        Pfinid.nlots.push(savedNlot);

        // Save the updated PfinidModel document
        await Pfinid.save();
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (id !== "pfini" && id !== "mp" && id !== "ticket" && id !== "em") {
      const allPfiniItems = await NlotdModel.findOne({ Nlot: id });
      console.log(allPfiniItems)
      res.status(200).json(allPfiniItems);
    } else {
      const allPfiniItems = await NlotdModel.find({ Category: id });
      res.status(200).json(allPfiniItems);
      console.log(allPfiniItems)
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedItem = await NlotdModel.findByIdAndRemove(req.params.id);
    ...
    const pfiniDocuments = await PfinidModel.find({ 'nlots._id': req.params.id });
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const existingItem = await NlotdModel.findById(itemId);
    ...
    if (originalCodeArticle !== CodeArticle) {
      ...
      const Nlot = await NlotdModel.findOne({ CodeArticle: CodeArticle });
      ...
      const originalPfini = await PfinidModel.findOne({ CodeArticle: originalCodeArticle });
...
router.get("/", async (req, res) => {
  try {
    ...
    if (nlot) {
      const nlotItem = await NlotdModel.findOne({ Nlot: nlot });
      ...
      const allNlots = await NlotdModel.find({});
      const result = [];
...
console.log(type)
    ...
    if (existingEntryIndex !== -1) {
      ...
      if (!iventoryDate || newEntryDate >= iventoryDate) {
        ...
        if (source === "floraison depot") {
          // If source is "floraison depot," add as sortie
...
const updateInventoryMiddleware = async (req, res, next) => {
  try {
    ...
    const nlot = await NlotdModel.findOne({ Nlot: id });
    ...
    // Update the corresponding Pfini document
    const pfini = await PfinidModel.findOne({ CodeArticle: nlot.CodeArticle });
